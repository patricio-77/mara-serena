...
function Login({ onLogin }) {
  ...
  const verificar = async () => {
    ...
    const { data: acceso } = await supabase.from("pacientes").select("*").eq("celular", num).single();
    setLoad(false);
    if(!acceso) { setPaso(2); return; }
    if(acceso.estado === "pendiente") { setPendiente(true); return; }
  ...
  if(pendiente) return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(180deg,#C8DBC5 0%,#F5EDE6 100%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, textAlign:"center" }}>
      ...
      <p style={{ fontSize:14, color:"#6B5C56", fontFamily:"'Raleway',sans-serif", lineHeight:1.6, maxWidth:280 }}>Tu solicitud fue enviada. Mara va a aprobar tu acceso y te va a avisar por WhatsApp.</p>
    </div>
  );
...
function TurnosDisponibles({ paciente }) {
  ...
  useEffect(() => {
    supabase.from("disponibilidad").select("*").eq("ocupado", false).order("fecha").order("hora").then(({ data }) => {
            ...
            <div key={fecha} style={{ marginBottom:16 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {sls.map(sl => {
                  ...
                  return (
                    <button key={sl.id} onClick={()=>{setDIdx(idx);setHoraSelec(sl.hora);}} style={{ padding:"10px 20px", borderRadius:50, border: sel?"1.5px solid #8FAF8A":"1.5px solid #DDD0C8", background: sel?"#8FAF8A":"#fff", color: sel?"#fff":"#2C2420", fontSize:13, fontFamily:"'Raleway',sans-serif", cursor:"pointer", fontWeight:600 }}>{formatHora(sl.hora)}</button>
                  );
                })}
              </div>
            </div>
...
function AdminTurnos() {
  ...
  const cargarTurnos = async () => {
    const { data } = await supabase.from("turnos").select("*, pacientes(nombre, apellido, celular)").order("fecha").order("hora");
  ...
  if(asignando && sel) {
    const filtrados = pacientes.filter(pac => (pac.nombre + " " + pac.apellido + " " + pac.celular).toLowerCase().includes(busqueda.toLowerCase()));
    return (
      <div style={{ paddingBottom:100 }}>
        <div style={{...S.header, display:"flex", alignItems:"center", gap:14}}>
          <div style={{...S.hAccent}}/>
          ...
          <div style={{ zIndex:1 }}><h1 style={{...S.hTitle}}>Asignar paciente</h1></div>
        </div>
        <div style={{ padding:20 }}>
    ...
    <div style={{ paddingBottom:100 }}>
      ...
      <div style={{ padding:20 }}>
        <div style={{...S.card}}>
          ...
          {sel.pacientes
            ? <><h2 style={{ fontSize:20, color:"#2C2420", fontWeight:600, margin:"12px 0 4px" }}>{sel.pacientes.nombre} {sel.pacientes.apellido}</h2>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}><IcoSVG name="wa" size={16} color="#8FAF8A"/><span style={{ fontSize:14, color:"#A89890", fontFamily:"'Raleway',sans-serif" }}>{sel.pacientes.celular}</span></div></>
            : <button style={{...S.btnG, marginTop:12}} onClick={async()=>{const{data}=await supabase.from("pacientes").select("*").eq("estado","aprobado");if(data)setPacientes(data);setAsignando(true);}}>+ Asignar paciente</button>
          }
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button style={{...S.btnS, flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:6}} onClick={()=>{setEditFecha(sel.fecha);setEditHora(sel.hora?.slice(0,5));setEditEstado(sel.estado);setEditando(true);}}>
            <IcoSVG name="edit" size={14} color="#2C2420"/> Editar
          </button>
          <button style={{...S.btnD, flex:1}} onClick={()=>cancelarTurno(sel.id)}>Cancelar turno</button>
        </div>
    ...
    <div style={{ paddingBottom:100 }}>
      ...
      <div style={{ padding:"16px 20px 0" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:16 }}>
          {[{lbl:"Total",val:turnos.length,col:"#2C2420"},{lbl:"Confirmados",val:turnos.filter(t=>t.estado==="confirmado").length,col:"#7BA68A"},{lbl:"Pendientes",val:turnos.filter(t=>t.estado==="pendiente").length,col:"#C8A46A"}].map(st=>(
            <div key={st.lbl} style={{...S.card, padding:"14px 10px", textAlign:"center", marginBottom:0}}>
              <p style={{ fontSize:26, color:st.col, margin:0, fontFamily:"'Raleway',sans-serif", fontWeight:800 }}>{st.val}</p>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:16, overflowX:"auto", paddingBottom:4 }}>
          {[{val:"todos",lbl:"Todos"},{val:"confirmado",lbl:"Confirmados"},{val:"pendiente",lbl:"Pendientes"},{val:"cancelado",lbl:"Cancelados"}].map(fi=>(
          ...
          <div key={fecha} style={{ marginBottom:8 }}>
            ...
            {trns.map(trn => (
              <div key={trn.id} style={{...S.card, cursor:"pointer", display:"flex", alignItems:"center", gap:12, padding:"14px 16px", marginBottom:8}} onClick={()=>setSel(trn)}>
                ...
                <div style={{ flex:1 }}>
                  {trn.pacientes ? <p style={{ fontSize:14, color:"#2C2420", margin:0, fontWeight:600 }}>{trn.pacientes.nombre} {trn.pacientes.apellido}</p> : <p style={{ fontSize:13, color:"#A89890", margin:0, fontFamily:"'Raleway',sans-serif", fontStyle:"italic" }}>Sin asignar</p>}
                </div>
...
function AdminDisponibilidad() {
  ...
  return (
    <div style={{ paddingBottom:100 }}>
      ...
      <div style={{ padding:20 }}>
        {Object.keys(porFecha).length === 0
          ? <p style={{ color:"#A89890", fontFamily:"'Raleway',sans-serif", textAlign:"center", marginTop:40 }}>No hay disponibilidad cargada.</p>
          : Object.entries(porFecha).map(([fecha, sls]) => (
            <div key={fecha} style={{...S.card}}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {sls.map(sl => (
                  <div key={sl.id} style={{ display:"flex", alignItems:"center", gap:6, padding:"5px 12px", borderRadius:50, background: sl.ocupado?"#FAE8E8":"#F5EDE6", border:"1px solid #DDD0C8" }}>
                    <span style={{ fontSize:12, fontFamily:"'Raleway',sans-serif", color: sl.ocupado?"#B86060":"#2C2420", fontWeight:600 }}>{formatHora(sl.hora)}</span>
...
function AdminPacientes() {
  ...
  const agregar = async () => {
    ...
    setLoad(true);
    const { error } = await supabase.from("pacientes").insert([{ celular: nuevoCel, nombre: nuevoNombre, apellido: nuevoApellido, estado:"aprobado" }]);
    setLoad(false);
    if(error) { alert("Error: " + error.message); return; }
  ...
  const aprobados = pacientes.filter(pac => pac.estado === "aprobado");
  const pendientes = pacientes.filter(pac => pac.estado === "pendiente");
    ...
    <div style={{ paddingBottom:100 }}>
      ...
      <div style={{ padding:"16px 20px 0" }}>
        <div style={{ display:"flex", gap:8, marginBottom:16 }}>
          {[{val:"aprobados",lbl:"Activas"},{val:"pendientes",lbl:"Pendientes"}].map(tb=>(
            <button key={tb.val} onClick={()=>setTab(tb.val)} style={{ padding:"7px 20px", borderRadius:50, border: tab===tb.val?"1.5px solid #8FAF8A":"1.5px solid #DDD0C8", background: tab===tb.val?"#8FAF8A":"#fff", color: tab===tb.val?"#fff":"#6B5C56", fontSize:12, fontFamily:"'Raleway',sans-serif", fontWeight:600, cursor:"pointer" }}>{tb.lbl}{tb.val==="pendientes"&&pendientes.length>0?` (${pendientes.length})`:""}</button>
          ))}
        </div>
